...
def enable_random_messages(update: Update, context: CallbackContext):
    queue.put({
        'type':'enablerandomchat',
        'value': update.message.chat_id
    })
    update.message.reply_text('ora farò il btdo inviando quando meno te lo aspetti messaggi e/o foto')

@restricted
def disable_random_messages(update: Update, context: CallbackContext):
    queue.put({
        'type':'disablerandomchat',
        'value': update.message.chat_id
    })
...
def stickers_inline_query_result(update: Update):
    update.inline_query.answer(get_sticker_inline_result(6), cache_time= INLINE_CACHE_TIME)
